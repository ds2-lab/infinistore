  ...
  if args.disk_source != "":
    if args.s3_source != "":
      ...
      trainset = DatasetDisk(
        filepaths = list(Path(os.path.join(args.disk_source, "training"))), s3_bucket = args.s3_train, label_idx=0, dataset_name=args.dataset, img_transform=normalize_cifar
      )
      if loadtestset:
        testset = DatasetDisk(
          filepaths = list(Path(os.path.join(args.disk_source, "test"))), s3_bucket = args.s3_test, label_idx=0, dataset_name=args.dataset, img_transform=normalize_cifar
        )
