...
do
  ...
  do
    ...
    RUNNING=`aws logs create-export-task --log-group-name $LAMBDA$LOG_PREFIX$i --from ${startTime} --to ${endTime} --destination "mason-leap-lab.datapool" --destination-prefix $FILE$PREFIX$LOG_PREFIX$i | grep taskId | awk -F \" '{ print $4 }'`
    if [ "$RUNNING" == "" ] ; then
      if [ k == 2 ] ; then
        echo "abort"
      else
